...
function buildLaneHeatmap(): LaneHourData[] {
  const basePatterns: number[] = [
    10, 8, 6, 7, 9, 20, 45, 68, 78, 72, 58, 48, 40, 38, 52, 61, 65, 55, 42, 32, 24, 18, 14, 11,
  ];
  const laneMultipliers: number[] = [0.9, 1.0, 0.85, 1.1, 1.2, 0.88, 0.95, 1.25, 0.75, 1.05, 1.15, 0.7];
  ...
  for (let lane = 1; lane <= 12; lane++) {
    for (let hour = 0; hour < 24; hour++) {
      data.push({
        lane,
        hour,
        congestion: Math.min(100, Math.round(basePatterns[hour] * laneMultipliers[lane - 1])),
      });
    }
  }
  return data;
...
export const bottlenecks: Bottleneck[] = [
  ...
  { id: 'b3', location: 'Lane 11 — Gate C',   severity: 'medium',   description: 'Gevaarlijke stoffen inspectie, ADR-vertraging', waitMinutes: 29, affectedTrucks: 4,  since: '09:02' },
];
...
export const aiInsights: AiInsight[] = [
  {
    ...
    description: 'Gate A Lane 7/8 toont structureel 34% meer congestion tussen 07:00–10:00. Spreiding naar Lane 4-5 in dat tijdslot bespaart gemiddeld 19 minuten per truck.',
    impact: '€18.600 / week',
    confidence: 87,
  },
  {
  ...
  },
  {
    ...
    impact: '−22% wachttijd op maandag',
    confidence: 94,
  },
  {
    id: 'ai4',
